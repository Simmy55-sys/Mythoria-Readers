...
export async function loginAction(credentials: LoginRequest) {
  ...
  if (setCookieHeader) {
    ...
    for (const cookieHeader of cookieHeaders) {
      ...
      // Only set the accessToken cookie
      if (cookieData.name === "accessToken") {
...
export async function registerAction(userData: RegisterRequest) {
  const { response, headers } = await apiClientManager.registerWithHeaders(
    userData
  );
